...
bool vertex_connected(adjList v_l, int v, int u);
int count_edge(adjList *graph);
...
int main()
{
    ...
    edge/=2;
    cout << endl
         << "all adjacent vertex for each vertex (using adjacent list) " << endl;
    ...
    {
        cout << endl;
        cout << "find shortest path in gragh from v to w => input 2" << endl
        ...
        case 3:
            ...
            if (isTree(graph))
                ...
                cout << "the graph is no tree" << endl;
            break;
        case 4:
            /* code */
            cout << "L= ";
...
int count_edge(adjList *graph)
{
    ...
    for (int i = 0; i < number_of_vertex; i++)
    {
        ...
        if (graph[i].getHead())
        {
            temp = graph[i].getHead();
            //cout<<"op= "<<temp->data<<endl;
            while (temp)
            {
                count++;
...
bool vertex_connected(adjList v_l, int v, int u) // if v have edge to u return true
{
    //cout<<"test begin"<<endl;
    adjList ::node *temp = v_l.getHead();
    bool yes = false;
